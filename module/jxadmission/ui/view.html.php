...
(
    ...
    to::suffix(
        ...
        hasPriv('jxadmission', 'edit') ? btn(set::icon('edit'), set::url(createLink('jxadmission', 'edit', "id={$row->id}")), $lang->edit) : null
    )
);
detailBody(sectionList(
    section(set::title($lang->jxadmission->common), tableData(
        item(set::name($lang->jxadmission->product), $product ? $product->name : '-'),
        item(set::name($lang->jxadmission->hospital), $hospital ? $hospital->name : '-'),
        item(set::name($lang->jxadmission->department), $row->department),
        item(set::name($lang->jxadmission->path), $row->path),
        item(set::name($lang->jxadmission->pharmacyDate), $row->pharmacyDate),
        item(set::name($lang->jxadmission->firstOrderDate), $row->firstOrderDate),
        item(set::name($lang->jxadmission->volume), $row->volume),
        item(set::name($lang->jxadmission->repurchase), zget($lang->jxadmission->repurchaseList, $row->repurchase)),
        item(set::name($lang->jxcore->progress), ($extra?->progress ?? 0) . '%'),
        item(set::name($lang->jxcore->blocker), $extra?->blocker ?? '-')
    )),
    section(set::title($lang->jxadmission->stages), $stageBlocks),
    section(set::title($lang->jxadmission->costs),
        div(setClass('mb-2 flex gap-4'), span($lang->jxcore->budget . '：' . ($summary->budget ?? $row->budget)), span($lang->jxcore->actual . '：' . ($summary->actual ?? 0)), $row->project ? btn(setClass('primary size-sm'), set::url(createLink('jxcore', 'addcost', "projectID={$row->project}")), set('data-toggle', 'modal'), $lang->jxcore->addcost) : null),
